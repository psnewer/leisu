...
class GOAL_MODEL_FILTER(ABSTRACT_FILTER):
	def __init__(self):
		self.name = 'GOAL_MODEL_FILTER'
		self.params = {}
		self.params['area'] = 1
	...
	def filter(self,df):
		...
		if len(df) > 0:
			...
			weights = gflags.FLAGS.extract_path + str(league_id) + '/' + serryname + '/snap/goal_rawall/_iter_200000.caffemodel'
