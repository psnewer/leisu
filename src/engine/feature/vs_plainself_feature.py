...
class VS_PLAINSELF_FEATURE(ABSTRACT_FEATURE):
	def __init__(self):
		self.name = 'VS_PLAINSELF_FEATURE'
		self.params = {}
	...
	def get_predict_currentMatchFeature(self,league_id,df_serry,serry_res):
		...
		for team in teams:
			...
			if num > 0:
				...
				res[team]['home_mean'] = home_mean
				res[team]['away_mean'] = away_mean
		return res		
	...
	def predict_pack(self,league_id,df_serry,serries,serry_feature,match_feature,feature_log):
		...
		if pre_serryid != 'none' and self.is_adjacent(serryid,pre_serryid,league_id):
			...
			for ind,row in df_serry.iterrows():
				...
				home_match_feature = []
				for i in range(0,self.params['levels']):
					home_match_feature.append(match_feature[home_team_id]['level'][i]['score'])
				home_match_feature.append(match_feature[home_team_id]['home_mean'])
				home_match_feature.append(match_feature[home_team_id]['away_mean'])
				away_match_feature = []
				for i in range(0,self.params['levels']):
					away_match_feature.append(match_feature[away_team_id]['level'][i]['score'])
				away_match_feature.append(match_feature[away_team_id]['home_mean'])
				away_match_feature.append(match_feature[away_team_id]['away_mean'])
				home_current_serryfeature = []
				for i in range(0,self.params['levels']):
					home_current_serryfeature.append(current_serry_feature[home_team_id]['level'][i]['score'])
				home_current_serryfeature.append(current_serry_feature[home_team_id]['home_mean'])
				home_current_serryfeature.append(current_serry_feature[home_team_id]['away_mean'])
				away_current_serryfeature = []
				for i in range(0,self.params['levels']):
					away_current_serryfeature.append(current_serry_feature[away_team_id]['level'][i]['score'])
				away_current_serryfeature.append(current_serry_feature[away_team_id]['home_mean'])
				away_current_serryfeature.append(current_serry_feature[away_team_id]['away_mean'])
				home_pre_feature = []
				for i in range(0,self.params['levels']):
				...
				home_res = [home_match_feature,home_current_serryfeature,home_pre_feature] 
				away_res = [away_match_feature,away_current_serryfeature,away_pre_feature]
				res_home = {}
				res_home['date'] = date
