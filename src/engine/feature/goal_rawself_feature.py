...
class GOAL_RAWSELF_FEATURE(ABSTRACT_FEATURE):
	def __init__(self):
		self.name = 'GOAL_RAWSELF_FEATURE'
		self.params = {}
		...
		for idx,serry in df_serry.iterrows():
			...
			for team in teams:
				...
				for date in team_dic:
					...
					level_res[level]['score'] += date_dic['goal']
		...
		for idx,serry in df_serry.iterrows():
			...
			for team in teams:
				...
				for date in dates:
					...
					_level_res = copy.deepcopy(level_res)
					res[serryid][team][date]['level'] = _level_res
	...
	def predict_pack(self,league_id,df_serry,serries,serry_feature,match_feature,feature_log):
		...
		if pre_serryid != 'none' and self.is_adjacent(serryid,pre_serryid,league_id):
			...
			for ind,row in df_serry.iterrows():
				...
				for i in range(0,self.params['levels']):
					home_current_serryscore.append(current_serry_feature[home_team_id]['level'][i]['score'])
					home_current_serrynum.append(current_serry_feature[home_team_id]['level'][i]['num'])
				home_current_serryscore.append(current_serry_feature[home_team_id]['home_score'])
				home_current_serrynum.append(current_serry_feature[home_team_id]['home_num'])
				home_current_serryscore.append(current_serry_feature[home_team_id]['away_score'])
				home_current_serrynum.append(current_serry_feature[home_team_id]['away_num'])
				away_current_serryscore = []
				away_current_serrynum = []
				for i in range(0,self.params['levels']):
					away_current_serryscore.append(current_serry_feature[away_team_id]['level'][i]['score'])
					away_current_serrynum.append(current_serry_feature[away_team_id]['level'][i]['num'])
				away_current_serryscore.append(current_serry_feature[away_team_id]['home_score'])
				away_current_serrynum.append(current_serry_feature[away_team_id]['home_num'])
				away_current_serryscore.append(current_serry_feature[away_team_id]['away_score'])
				away_current_serrynum.append(current_serry_feature[away_team_id]['away_num'])
				home_pre_score = []
				home_pre_num = []
				for i in range(0,self.params['levels']):
				...
				home_res = [[home_match_score,home_current_serryscore,home_pre_score],[home_match_num,home_current_serrynum,home_pre_num]]
				away_res = [[away_match_score,away_current_serryscore,away_pre_score],[away_match_num,away_current_serrynum,away_pre_num]]
				res_home = {}
				res_home['date'] = date
