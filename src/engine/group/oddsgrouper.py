...
class OddsGrouper():
	def __init__(self,tester_creator,experiments,condition):
		...
		self.plain_strena = Plain_STrena()
		...
		if gflags.FLAGS.update and not os.path.exists(self.group_directory):
			gflags.FLAGS.update = False
			gflags.FLAGS.test = True
		os.system(r'mkdir %s'%self.group_directory)
		self.test = gflags.FLAGS.test
		self.update = gflags.FLAGS.update

	def group(self):
		for cond in self.conditions:
			gflags.FLAGS.test = self.test
			gflags.FLAGS.update = self.update
	...
	def process(self,cond):
		...
		if gflags.FLAGS.update and not os.path.exists(group_directory):
			gflags.FLAGS.update = False
			gflags.FLAGS.test = True
		...
		if os.path.exists(test_res_file):
			...
			if gflags.FLAGS.test:
				...
				for row in kind_trend:
					...
					experiment_ids = row['experiment']
					exp = self.trend_experiments[trend_experiment]
					features = exp['feature']
					filters = exp['filter']
					trent_res = self.trent_creator.test(filters,dic_res,experiment_ids,test_id=trend_experiment)
					df_trent = pd.DataFrame(trent_res)
					df_trena = df_trena.append(df_trent)
				df_trena.to_csv(group_directory + '/trent_res.txt')
				df_trena = pd.DataFrame.from_csv(group_directory + '/trent_res.txt')
			...
			for key in trena_dic:
				...
				for test_id_str in te: 
					...
					experiment_ids = te[test_id_str].keys()
					experiment_ids = map(int,experiment_ids)
					...
					if trenf_res is not None and len(trenf_res) > 0:
						...
						for experiment_id in experiment_ids:
							trena_thresh = te[test_id_str][unicode(experiment_id)]
							if experiment_id in filtered_experiments:
								if experiment_id not in filtered_exps:
									filtered_exps[experiment_id] = {}
									filtered_exps[experiment_id]['test_id'] = {}
									filtered_exps[experiment_id]['test_id'][test_id] = trena_thresh
								elif test_id not in filtered_exps[experiment_id]['test_id']:
									filtered_exps[experiment_id]['test_id'][test_id] = trena_thresh
								elif trena_thresh < filtered_exps['test_id'][test_id]:
									filtered_exps[experiment_id]['test_id'][test_id] = trena_thresh
