...
class VS_RAWTAW_EXTRACTOR(ABSTRACT_EXTRACTOR):
	def __init__(self):
		self.name = 'VS_RAWTAW'
		self.params = {}
	...
	def analyze_procedure(self, procedure, team, is_home):
		...
		for event in procedure:
			...
			if score_home > 0 or score_away > 0:
				...
				if score_home == score_away:
					...
					if raw_draw is None:
						raw_draw,taw_draw = (1.0 + 1.0/(float(event['away']) - 1.0) if event['away'] else math.nan,event['away']) if is_home else (1.0 + 1.0/(float(event['home']) - 1.0) if event['home'] else math.nan,event['home'])
		...
		if bought and draw_at_some_point and team_lose:
			rawtaw = 1

		return raw,taw,raw_win,taw_win,raw_draw,taw_draw,rawtaw
	...
	def generate_rows(self, df):
		...
		for idx, row in df.iterrows():
			...
			home_raw,home_taw,home_raw_win,home_taw_win,home_raw_draw,home_taw_draw,home_rawtaw = self.analyze_procedure(procedure, home_team, is_home=True)
			away_raw,away_taw,away_raw_win,away_taw_win,away_raw_draw,away_taw_draw,away_rawtaw = self.analyze_procedure(procedure, away_team, is_home=False)
		...
		return pd.DataFrame(rows, columns = ['team','raw','taw','rawtaw','raw_win','taw_win','raw_draw','taw_draw','league','season','serryid','serryname','stage','date','home_team','away_team','score','procedure'])
