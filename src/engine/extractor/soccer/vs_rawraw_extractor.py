	...
	def analyze_procedure(self, procedure, score, is_home):
		...
		for event in procedure:
			...
			if score_home > 0 or score_away > 0:
				...
				if score_away == 0 and score_home == 1 and not is_home:
					bought = True
					if not draw_0:
						draw_0 = float(event['draw']) if event['draw'] else math.nan
				elif score_away == 0 and score_home == 2 and time <= 45 and not is_home:
					bought = True
					if not draw_1:
						draw_1 = float(event['draw']) if event['draw'] else math.nan
				elif score_home == 0 and score_away == 1 and is_home:
					bought = True
					if not draw_0:
						draw_0 = float(event['draw']) if event['draw'] else math.nan
				elif score_home == 0 and score_away == 2 and time <= 45 and is_home:
					bought = True
					if not draw_1:
						draw_1 = float(event['draw']) if event['draw'] else math.nan
