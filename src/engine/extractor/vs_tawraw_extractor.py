...
class VS_TAWRAW_EXTRACTOR(ABSTRACT_EXTRACTOR):
	def __init__(self):
		self.name = 'VS_TAWRAW'
		self.params = {}
	...
	def analyze_procedure(self, procedure, team, is_home):
		...
		for event in procedure:
			...
			if score_home > 0 or score_away > 0:
				...
				if raw_win is None:
					raw_win,taw_win = (event['home'],1.0 + 1.0/(float(event['home']) - 1.0) if event['home'] else math.nan) if is_home else (event['away'],1.0 + 1.0/(float(event['away']) - 1.0) if event['away'] else math.nan)
				...
				if score_home == score_away:
					...
					if raw_draw is None:
						raw_draw,taw_draw = (event['home'],1.0 + 1.0/(float(event['home']) - 1.0) if event['home'] else math.nan) if is_home else (event['away'],1.0 + 1.0/(float(event['away']) - 1.0) if event['away'] else math.nan)
		...
		if bought and team_won and draw_at_some_point:
			tawraw = 1

		return raw,taw,raw_win,taw_win,raw_draw,taw_draw,tawraw
	...
	def generate_rows(self, df):
		...
		for idx, row in df.iterrows():
			...
			home_raw,home_taw,home_raw_win,home_taw_win,home_raw_draw,home_taw_draw,home_tawraw = self.analyze_procedure(procedure, home_team, is_home=True)
			away_raw,away_taw,away_raw_win,away_taw_win,away_raw_draw,away_taw_draw,away_tawraw = self.analyze_procedure(procedure, away_team, is_home=False)
		...
		return pd.DataFrame(rows, columns = ['team','raw','taw','tawraw','raw_win','taw_win','raw_draw','taw_draw','league','season','serryid','serryname','stage','date','home_team','away_team','score','procedure'])
