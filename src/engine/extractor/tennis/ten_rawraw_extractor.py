...
class TEN_RAWRAW_EXTRACTOR(object):
	def __init__(self):
	...
	def analyze_match_scores(self, score_list, is_home):
		...
		profit = 0
		if len(results):
			if results[0]['team_broken'] or (not results[0]['team_win'] and not results[0]['team_break']):
				score = -1
			if (results[0]['team_broken'] and results[0]['team_broken_back']):
				score = 1
			elif len(results) > 1:
				if (results[0]['team_broken']):
					if (results[1]['team_break']):
						score = 0.5
					elif (results[1]['team_broken'] and results[1]['team_broken_back']):
						score = -0.5
				elif (not results[0]['team_win']) and (not results[0]['team_break']) and (not results[0]['team_broken']):
					if (results[1]['team_break']):
						score = 1.5
					elif (results[1]['team_win']):
						score = 2
			profit = score
			if score > 0:
