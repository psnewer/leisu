...
class VS_RAWRAW_EXTRACTOR(ABSTRACT_EXTRACTOR):
	...
	def analyze_procedure(self, procedure, team, is_home):
		...
		return raw,taw,raw_win,taw_win,raw_draw,taw_draw,rawraw
	...
	def generate_rows(self, df):
		...
		for idx, row in df.iterrows():
			...
			home_raw,home_taw,home_raw_win,home_taw_win,home_raw_draw,home_taw_draw,home_rawraw = self.analyze_procedure(procedure, home_team, is_home=True)
			away_raw,away_taw,away_raw_win,away_taw_win,away_raw_draw,away_taw_draw,away_rawraw = self.analyze_procedure(procedure, away_team, is_home=False)
		...
		return pd.DataFrame(rows, columns = ['team','raw','taw','rawraw','raw_win','taw_win','raw_draw','taw_draw','league','season','serryid','serryname','stage','date','home_team','away_team','score','procedure'])
