...
class TEN_RAWRAW_TESTER(object):
	def __init__(self):
	...
	def process(self,seasons,ext_dir):
		for season in seasons:
			...
			df['seal'] = df['selected'].apply(self.extract_seal_values)
	...
	def extract_raw_values(self,selected_list):
		...
		return [item['score'] for item in selected_list]
	
	def extract_odds_values(self,selected_list):
		selected_list = json.loads(selected_list)
		return [item['profit'] for item in selected_list]
	
	def extract_seal_values(self,selected_list):
		selected_list = json.loads(selected_list)
		return [item['seal'] for item in selected_list]
