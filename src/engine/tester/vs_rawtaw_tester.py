...
class VS_RAWTAW_TESTER(ABSTRACT_TESTER):
	def __init__(self):
		self.name = 'VS_RAWTAW'
		self.params = {}
	...
	def extract_raw_values(self,selected_list):
		...
		return [item['rawtaw'] for item in selected_list]

	def extract_odds_values(self,selected_list):
		selected_list = json.loads(selected_list)
		return [np.nan_to_num([item['raw_win'],item['taw_draw']], nan=0).tolist() for item in selected_list]
