...
class VS_RAWRAW_TESTER(ABSTRACT_TESTER):
	def __init__(self):
		self.name = 'VS_RAWRAW'
		self.params = {}
	...
	def extract_raw_values(self,selected_list):
		...
		return [item['rawraw'] for item in selected_list]

	def extract_odds_values(self,selected_list):
		selected_list = json.loads(selected_list)
		return [np.nan_to_num([item['raw_win'],item['raw_draw']], nan=0).tolist() for item in selected_list]
