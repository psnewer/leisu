...
modules = glob.glob(os.path.join(current_dir, "*_tester.py"))
...
for module_path in modules:
    ...
    imported_module = importlib.import_module(f'tester.{module_name}')
    # 查找模块中的所有类，并找到 *_FEATURE 类
    for attribute_name in dir(imported_module):
        attribute = getattr(imported_module, attribute_name)
        if isinstance(attribute, type) and attribute_name.endswith('_TESTER'):  # 匹配 *_FEATURE 类
